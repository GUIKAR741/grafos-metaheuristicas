...
def plotar(individuo):
    ...
    corteA = 1
    i1 = individuo[0][0]
    a1 = arestas[i1] if individuo[1][0] == 0 else arestas[i1][::-1]
    if a1[0] != (0.0, 0.0):
        x1.append(0.0)
        y1.append(0.0)
        x1.append(a1[0][0])
    ...
    for i in range(len(individuo[0]) - 1):
        ...
        a1 = arestas[i1] if individuo[1][i] == 0 else arestas[i1][::-1]
        a2 = arestas[i2] if individuo[1][i + 1 if i + 1 <
                                         len(individuo[0]) else 0] == 0 else arestas[i2][::-1]
        x1, y1, x, y = [], [], [], []
        if a1[1] != a2[0]:
            x1.append(a1[1][0])
            y1.append(a1[1][1])
...
for i in range(n):
    ...
    # arestas.append([(a[2], a[3]), (a[0], a[1])])
...
def genIndividuo():
    ...
    #         individuo.append(i * 2)
    #     else:
    #         individuo.append(i * 2 + 1)
    # random.shuffle(individuo)
    v = [random.randint(0, 1) for i in range(len(arestas))]
    random.shuffle(v)
    return random.sample(range(len(arestas)), len(arestas)), v
...
# Attribute generator
toolbox.register("indices", genIndividuo)
...
def evalCorte(individuo, pi=1, mi=5):
    """Efetua o Corte das Arestas.

    args:
        pi velocidade do corte
        mi velocidade do deslocamento

    posicao i for par então direção normal
    se i for impar direção contraria
    """
    dist = 0
    i1 = individuo[0][0]
    a1 = arestas[i1] if individuo[1][0] == 0 else arestas[i1][::-1]
...
def main(pop=10000, CXPB=0.75, MUTPB=0.1, NGENSEMMELHORA=100, arq=None):
    """."""
